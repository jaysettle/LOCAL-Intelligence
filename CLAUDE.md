...
- **Two REPLs.** `_repl_plain` (default) is a simple synchronous line-by-line reader — reliable, renders answers cleanly. It reads with `input()`, so a *pasted* block would arrive one line per turn; `_read_prompt` drains the console buffer after the first line (`msvcrt.kbhit` / `select`) and joins the paste into one prompt — **only when stdin is a tty**. Piped stdin must stay strictly line-by-line: scripts and the live tests (`printf '/skill\n/exit\n' | gemma go`) depend on it. Commands never absorb following lines. `_repl_interactive` (`--live` / `live_repl: true`) adds a type-ahead queue, Esc-cancel, and a live GPU/CPU status line via `prompt_toolkit` + `patch_stdout` + a worker thread. **The live REPL is experimental and off by default** (see pitfalls).
...
- **Every external command in `install.ps1` goes through `Invoke-Native`.** This bug class shipped twice: PowerShell 5.1 wraps a native command's stderr in an ErrorRecord that THROWS under `$ErrorActionPreference = "Stop"`, so *any* chatty tool aborts the install. First it was git (`dubious ownership`); then, after fixing only git, it was the Microsoft Store `python.exe` alias stub, whose "Python was not found" is a correct answer to "is python here?" — and the installer died on it without ever reaching the step that installs Python. `Invoke-Native` relaxes the preference and returns `@{ Output; Code }`. **Judge by `Code`, never by whether stderr spoke, and never add a bare `& tool ... 2>&1` to this script.** The lesson generalises: when you fix one instance of a pattern in this file, grep for the rest of them.
...
- **rich output from a background thread under `patch_stdout` is unreliable.** Symptoms seen: color codes leaking as literal `?[2m ... ?[0m`, then the answer vanishing entirely. Root cause: rich's terminal control from a non-main thread fights prompt_toolkit's screen management. The worker path now renders with plain `print(flush=True)` (`_consume_plain`), not rich. The remaining tangle had a precise cause, seen on a real Windows terminal: `patch_stdout` renders a *complete* line cleanly above the prompt, but a flushed *partial* line is written on the prompt's own row and redrawn on every refresh — so streaming tokens with `end=""` left the answer's last words stuck bottom-left while blank space grew. **`_consume_plain` must only ever print whole lines** (buffer, emit at newlines, soft-wrap past ~100 chars); a test asserts `print` is never called with `end=""` there. The live REPL stays opt-in and the plain REPL the default. **Do not make the live REPL the default without validating on a real Windows terminal** (it cannot be verified from a non-TTY dev environment).
